...
// posted message.
figma.ui.onmessage =  (msg: {type: string, data: object}) => {
  // One way of distinguishing between different types of messages sent from
  // your HTML page is to use an object with a "type" property like this.
  if (msg.type === 'create-shapes') {
    console.log('create-shapes')
  } else if (msg.type === 'set-prop') {
    console.log("onmessage set-prop", msg);
        ...
        if (fill.boundVariables?.color){
          ...
          if (variableP) {
            variableP.then(variable=>{
              console.log("使用了颜色变量", variable?.name);
              let resultComponentVariableId = `颜色变量${variable?.name}`; 
